    ...
    <div class="row invoice-info">
        ...
        <div class="col-sm-4 invoice-col">
            <div class="form-group">
                ...
                {!! Form::text('phone', $order->user->phone, ['class' => 'form-control', 'readonly']) !!}
            </div>
            <div class="form-group">
                {!! Form::label('email', 'E-mail', ['class' => 'control-label']) !!}
                {!! Form::text('email', $order->user->email, ['class' => 'form-control', 'readonly']) !!}
            </div>
            {{--
            <div class="form-group {{ $errors->has('comment') ? 'has-error' : ''}}">
                {!! Form::label('comment', 'Комментарий', ['class' => 'control-label']) !!}
                {!! Form::textarea('comment', null, ['class' => 'form-control', 'rows' => 12, 'cols' => 20]) !!}
    ...
    <div class="row invoice-info">
        <div class="col-sm-4 invoice-col">
            ...
            <div class="form-group {{ $errors->has('data.payment.method') ? 'has-error' : ''}}">
                ...
                {!! Form::select('data[payment][method]', ['' => '---'] + array_column(json_decode(variable('payment_methods', '[]'), true), 'value', 'key'), $order->data['payment']['method'] ?? '', ['class' => 'form-control  select2', 'data-minimum-results-for-search' => '-1']) !!}
{{--                {!! Form::select('data[payment][method]', ['' => '---', 'upon_receipt' => 'Оплата при получении', 'yandex' => 'Яндекс.Деньги: Выдача заказа по паспорту'], $order->data['payment']['method'] ?? '', ['class' => 'form-control  select2', 'data-minimum-results-for-search' => '-1']) !!}--}}
            ...
            <div class="form-group {{ $errors->has('data.delivery.tariff') ? 'has-error' : ''}}">
                {!! Form::label('data[delivery][tariff]', 'Тариф (CDEK)', ['class' => 'control-label']) !!}
                {!! Form::select('data[delivery][tariff]', ['' => '---'] + \App\Models\Shop\Order::$cdekTarifs, $order->data['delivery']['tariff'] ?? '', ['class' => 'form-control  select2', 'data-minimum-results-for-search' => '-1']) !!}
        ...
        {{--files start--}}
    {{--
    @if($order->getMedia('files')->count())
        <div class="row">
            <div class="col-lg-12">
                <h3>Файли замовника</h3>
                <div class="table-responsive">
                    <table class="table table-striped">
                        ...
                        <tbody>
                        @foreach($order->getMedia('files') as $media)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td><a href="{{ $media->getUrl() }}" target="_blank" download>{{ $media->name }}</a></td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
        ...
        <div class="row">
            <div class="col-xs-12 table-responsive">
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                    @foreach($order->data['sales'] ?? []  as $key => $sale)
                        @if($key === 'promocode')
                        @else
                        <tr>
                            <td>{{ $sale['id'] }}</td>
                            ...
                            <td>@if($sale['discount_type'] == \App\Models\Shop\Sale::DISCOUNT_TYPE_SUM){{ Currency::format($sale['discount'] ?? 0, 'RUB') }}@else {{$sale['discount'] ?? 0}}% @endif</td>
                        </tr>
                        @endif
                    @endforeach
                    </tbody>
        ...
        <div class="col-xs-12">
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tr>
                        <th>Скидка:</th>
                        <td>{{ Currency::format($order->data['purchase']['discount'] ?? 0, 'RUB') }}</td>
                    </tr>
                    <tr>
                        <th>Доставка:</th>
                        <td>{{ Currency::format($order->data['purchase']['delivery'] ?? 0, 'RUB') }}</td>
                    </tr>
                    <tr>
                        <th>К оплате:</th>
                        <td>{{ Currency::format($order->price + ($order->data['purchase']['delivery'] ?? 0) - ($order->data['purchase']['discount'] ?? 0), 'RUB') }}</td>
                    </tr>
                </table>
            </div>
        </div>
