...
<div class="row">
    <div class="col-lg-6">
        ...
        <div class="form-group {{ $errors->has('data.applying') ? 'has-error' : ''}}">
            {!! Form::label('data[applying]', 'Применение', ['class' => 'control-label']) !!}
    ...
    <div class="col-lg-6">
        ...
        @if(isset($product) && $product->group->default_product_id == $product->id && ! Route::currentRouteNamed('*create') || empty($product))
        ...
        @include('admin.fields.field-treeview', [
            ...
            <div class="box box-warning box-solid">
                <div class="box-header">
                    <h3 class="box-title">Параметры для службы доставки</h3>
                </div>
                <div class="box-body">

                    <div class="form-group {{ $errors->has('data.delivery_info') ? 'has-error' : ''}}">
                        {!! Form::label('data[delivery_info]', 'Информация о доставке', ['class' => 'control-label']) !!}
                        {!! Form::text('data[delivery_info]', isset($product) ? ($product->data['delivery_info'] ?? '') : null, ['class' => 'form-control', ]) !!}
                        {!! $errors->first('data.delivery_info', '<p class="help-block">:message</p>') !!}
                    </div>
            ...
            <div class="box box-warning box-solid">
                ...
                <div class="col-md-4">
                    @include('admin.fields.field-text', [
                        ...
                        'field_name' => 'data[front][tab_description]',
                        'value' => isset($product) ? ($product->data['front']['tab_description'] ?? '') : '',
                    ])
                </div>
                <div class="col-md-4">
                    @include('admin.fields.field-text', [
                        'label' => 'Tab 2 ("Применение")',
                        'field_name' => 'data[front][tab_applying]',
                        'value' => isset($product) ? ($product->data['front']['tab_applying'] ?? '') : '',
                    ])
                </div>
                <div class="col-md-4">
                    @include('admin.fields.field-text', [
                        'label' => 'Tab 3 ("Состав")',
                        'field_name' => 'data[front][tab_composition]',
                        'value' => isset($product) ? ($product->data['front']['tab_composition'] ?? '') : '',
                    ])
                </div>
                </div>
            </div>
