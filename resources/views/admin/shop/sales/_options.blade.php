    ...
    <div class="col-lg-6">
        ...
        @if(in_array($sale->type, [\App\Models\Shop\Sale::TYPE_PRODUCT, \App\Models\Shop\Sale::TYPE_PROM_CODE_PRODUCT, \App\Models\Shop\Sale::TYPE_PROM_CODE_DISCOUNT_SUM_ORDER]))
            @include('admin.fields.field-radio-group', [
                ...
                'attributes' => [\App\Models\Shop\Sale::DISCOUNT_TYPE_PERCENT => 'Скидка "Процент от суммы" (%)', \App\Models\Shop\Sale::DISCOUNT_TYPE_SUM => 'Скидка "Фиксированная сумма" (Руб.)',]
            ])
        ...
        @if(in_array($sale->type, [\App\Models\Shop\Sale::TYPE_PROM_CODE_PRODUCT_PRESENT]))
            @include('admin.fields.field-select2-ajax-autocomplete', [
                ...
                'field_name' => 'data[product_present]',
                'multiple' => 0,
                'disabled' => 0,
                'selected' => isset($sale) && isset($sale->data['product_present']) ? \App\Models\Shop\Product::where('id', $sale->data['product_present'])->get()->pluck('name', 'id')->toArray() : null,
                //'old' => old('product_present')
            ])
        @endif

        @if(in_array($sale->type, [\App\Models\Shop\Sale::TYPE_FREE_SHIPPING_CONDITIONS, \App\Models\Shop\Sale::TYPE_PROM_CODE_PRODUCT, \App\Models\Shop\Sale::TYPE_PROM_CODE_DISCOUNT_SUM_ORDER,]))
        ...
        @if(in_array($sale->type, [\App\Models\Shop\Sale::TYPE_FREE_SHIPPING_CONDITIONS,]))
            <div class="">
                <label>
                    ...
                    <input type="checkbox" value="1" name="data[only_delivery_pwz]" @if(!empty($sale->data['only_delivery_pwz'])) checked @endif> + если выбрано "Доставка до пункта самовывоза (136)"
                </label>
            </div>
            <div class="">
                <label>
                    <input type="hidden" value="0" name="data[tariff_zone_without_6_7]">
                    <input type="checkbox" value="1" name="data[tariff_zone_without_6_7]" @if(!empty($sale->data['tariff_zone_without_6_7'])) checked @endif> + актуально для всех регионов, кроме 6 и 7 зоны (Сибирь и Дальний Восток)
                </label>
            </div>
        @endif

        @if(in_array($sale->type, [\App\Models\Shop\Sale::TYPE_PROM_CODE_PRODUCT, \App\Models\Shop\Sale::TYPE_PROM_CODE_FREE_ORDER, \App\Models\Shop\Sale::TYPE_PROM_CODE_DISCOUNT_SUM_ORDER, \App\Models\Shop\Sale::TYPE_PROM_CODE_PRODUCT_PRESENT]))
