...
if (! function_exists('cdek_str_delivery_info')) {
     ...
     * @return string
     */
    function cdek_str_delivery_info($response, string $prefix = '', $forcePrice = null, $options = [])
    {
        ...
        if ($response instanceof \CdekSDK\Responses\CalculationResponse) {
            try {
                ...
                if (isset($options['city_id']) && $options['city_id'] == 44) {
                    $dMinTerm = 1;
                    $dMaxTerm = $dMaxTerm + 1;
                }

                $dPrice = $forcePrice !== null ? $forcePrice : $response->getPrice();
                $dMaxTermStr = $dMaxTerm > 0 && $dMaxTerm != $dMinTerm ? "- $dMaxTerm " : "";
                $dStr .= ": Срок доставки $dMinTerm $dMaxTermStr ".trans_choice('front.день', $dMaxTerm).". - $dPrice";
