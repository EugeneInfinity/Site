...
{
    public function updateOrCreateForEntity($node, Request $request)
    {
        $metaTag = $node->metaTag()->updateOrCreate([], $request->get('meta_tag', []) + [
            'og_title' => $request->input('meta_tag.title'),
            'og_description' => $request->input('meta_tag.description'),
        ]);
    ...
    public function saveOgImageForPath($metaTag, Request $request)
    {
        $ogImgName = md5($metaTag->id);
        $fileObj = null;
