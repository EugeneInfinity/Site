...
@section('content')
    ...
    <p><b>Тип оплаты:</b> {{ array_column(json_decode(variable('payment_methods', '[]'), true), 'value', 'key')[$order->data['payment']['method'] ?? ''] ?? '' }}</p>

    @php($tariffs = ['' => '---', '136' => 'Доставка до пункта самовывоза (136)', '137' => 'Доставка "до двери" (137)'])
    <p><b>Тариф СДЕК:</b> {{ $tariffs[$order->data['delivery']['tariff'] ?? ''] ?? '' }}</p>
