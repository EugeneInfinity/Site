...
{
    use HasMediaTrait, UrlAliasable, UrlAliasGenerator, Sortable, Filterable, Navigable, Metatagable;

    /**
     * Тип акции.
     * Тип способа (условие) начисления акционной скидки.
     */
    /** @var int Скидка на товары - старая/новая цена */
    const TYPE_PRODUCT = 1;
    /** @var int Скидка по промокоду на товар */
    const TYPE_PROM_CODE_PRODUCT = 2;
    /** @var int Бессплатная доставка (с условиями) */
    const TYPE_FREE_SHIPPING_CONDITIONS = 3;
    /** @var int Бессплатная доставка по промокоду */
    const TYPE_PROM_CODE_FREE_ORDER = 4;
    /** @var int Скидка при сумме заказа */
    const TYPE_PROM_CODE_DISCOUNT_SUM_ORDER = 5;
    /** @var int Товар в подарок по промокоду*/
    ...
    public static $types = [
        ...
        self::TYPE_FREE_SHIPPING_CONDITIONS => 'Бессплатная доставка (с доп. условиями)',
    ...
    {
        return $query->where('publish', 1)->where(function ($sales) {
            $sales->where('dateless', 1)->orWhere(function ($sales2) {
    ...
    /**
     * @param \App\Models\Shop\Media|null $media
